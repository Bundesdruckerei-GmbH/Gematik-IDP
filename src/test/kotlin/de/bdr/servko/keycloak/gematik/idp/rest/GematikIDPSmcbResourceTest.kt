...
internal class GematikIDPSmcbResourceTest : GematikIDPEndpointBaseTest() {
    ...
    private val underTest = GematikIDPSmcbResource(
    ...
    fun `startAuth - returns correct form response`() {
        ...
        verify(authSessionMock).setAuthNote("GEMATIK_IDP_STEP", "REQUESTED_SMCB_DATA")
    }
    ...
    fun `nextStep - SMC-B data saved in auth session`() {
        ...
        val hbaTelematikID = "5-SMC-B-Testkarte-883110000129071"
        ...
        assertThat(contextData[ContextData.CONTEXT_HBA_TELEMATIK_ID.name]).isEqualTo("UNKNOWN")
        assertThat(contextData[ContextData.CONTEXT_HBA_PROFESSION_OID.name]).isEqualTo("UNKNOWN")
        assertThat(contextData[ContextData.CONTEXT_HBA_GIVEN_NAME.name]).isEqualTo("UNKNOWN")
        assertThat(contextData[ContextData.CONTEXT_HBA_FAMILY_NAME.name]).isEqualTo("UNKNOWN")
    ...
    @Test
    fun `result - received HBA data successfully - error saved in session, because unsupported card type`() {
        // arrange
        isHba = false
    ...
    private fun assertAuthenticatorUrl(authenticatorUrl: URI) {
        ...
        assertThat(challengeQueryParams["cardType"]).isEqualTo("SMC-B")
