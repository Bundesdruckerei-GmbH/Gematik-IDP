...
internal class SmcbUserAttributeMapperTest {
    ...
    fun setup() {
        ...
        context = BrokeredIdentityContext("id", idpConfig).apply {
    ...
    fun getConfigProperties() {
        ...
        assertThat(claimAttribute.options).hasSize(5)
            .containsExactly(
                GematikClaim.TELEMATIK_ID.name,
                GematikClaim.PROFESSION_OID.name,
                GematikClaim.ORGANIZATION_NAME.name,
    ...
    fun updateBrokeredUser_removeAttribute() {
        val user: UserModel = mock()
        context.contextData[ContextData.CONTEXT_SMCB_TELEMATIK_ID.name] = null
    ...
    @Test
    fun getId() {
        assertThat(objectUnderTest.id).isEqualTo("gematik-idp-smcb-user-mapper")
    }

    @Test
    fun getHelpText() {
        assertThat(objectUnderTest.helpText).isEqualTo("Import the claim of the SMCB card, if it exists, into the specified user attribute.")
    }

    @Test
    fun getDisplayType() {
        assertThat(objectUnderTest.displayType).isEqualTo("Gematik SMCB Claim Attribute")
    }
